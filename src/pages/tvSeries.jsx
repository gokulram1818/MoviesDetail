...
function Movies() {
  ...
  useEffect(()=>{
    fetch("https://api.themoviedb.org/3/trending/tv/day?api_key=74d1441879935853e509d09572e6f471")
    .then((res)=>res.json())
  ...
  useEffect(() => {
    if (!Query.trim()) {
      ...
      fetch(`https://api.themoviedb.org/3/search/tv?api_key=74d1441879935853e509d09572e6f471&query=${Query}`)
        .then((res) => res.json())
        .then((data) => setSearchMovie(data.results))
    }
  }, [Query])
  ...
  >
    {PopularMovies.map((movie)=>(
      <Link to={`/tv/${movie.id}`} key={movie.id} className="nav-link">
    ...
    <input 
    type="text"
    className=" form-control w-25 my-5"
